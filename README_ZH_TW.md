...
Go Web Frame 是一個 opinionated 的 Web 框架，通過基於組件的設計強制實現清晰的架構。它內建了依賴注入、類型安全 ORM、資料庫整合等特性。

**核心優勢**：本框架整合了 Go 生態中優秀的開源套件和框架，採用最佳實踐有機配合，是目前最佳的組合方案，可幫助開發者在短時間內快速成型專案，大幅提升開發效率。
...
如果你正在尋找：
- 一個**功能完整**且**開箱即用**的 Go Web 框架
- 整合了**生產驗證**的優秀組件，無需反覆調研選型
- **泛型支援**的類型安全 ORM，減少樣板程式碼
- **清晰架構**，便於維護和團隊協作
- **快速開發**，縮短從想法到產品的時間

**Go Web Frame 是你的最佳選擇！**

## 🌟 特性

- 🎯 **類 MVC 架構**：清晰的關注點分離，包含服務、控制器和模型
- ⚡ **類型安全泛型 ORM**：零樣板程式碼 CRUD 操作，基於泛型實現，無反射效能損耗
- 🧩 **依賴注入**：內建 DI 容器，管理組件生命週期
- 📦 **資料庫整合**：SQLite、MySQL、PostgreSQL、Redis 支援，可擴展的資料庫抽象層（基於 GORM）
- 🏊 **連線池配置**：可配置連線池參數（`max_open_conns`、`max_idle_conns`、`conn_max_lifetime`）
- 🛠️ **組件系統**：可重用組件，包括快取、限流、驗證碼、二維碼生成、定時任務、輸入驗證
- 🌐 **RESTful 支援**：簡化的 REST 控制器實現
- ⚙️ **自動配置**：從 JSON、YAML 或 TOML 檔案自動載入配置
- 📝 **進階日誌**：基於 Zap 的結構化日誌，支援日誌輪轉
- 🔄 **後台任務**：內建的後台任務執行器系統
- 🛡️ **請求過濾**：HTTP 中間件/過濾器系統，處理橫切關注點
- 🏷️ **路由元數據**：支援 `.WithMeta()` 為路由附加自定義元數據，實現靈活的路由級別認證、權限檢查等
